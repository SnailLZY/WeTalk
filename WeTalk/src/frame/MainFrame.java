...
public class MainFrame extends JFrame implements MouseListener, ActionListener, MouseInputListener, KeyListener {
    ...
    五、设置
     */
    private JFrame frame;//和签名相关，移入就显示，移出就删除
    ...
    private CardLayout cl;//这里用的是CardLayout，用来配合不同的输入显示相应的结果
   ...
   /* public static void main(String[] args) {
    ...
    public void setFunc_menus() {
       ...
       // func_menus.setLayout(null);
        ...
        func_menus.setBounds(0, 140, 360, 30);
      ...
      //  func_menus.add(groupButtonGroup);
        ...
        func_menus.setBackground(new Color(22, 212, 107));
        this.add(func_menus);
    ...
    public void updateFirendList(Message m) {
        ...
        for (int i = 0; i < friendCount; i++) {
            ...
            jLabels[i].setIcon(new ImageIcon(new ImageIcon(("src/frame/image/friendListIconTemp.jpg")).getImage().getScaledInstance(60, 60, Image.SCALE_DEFAULT)));
    ...
    public void setFriendList() {
        ...
        jScrollPane1.setBounds(new Rectangle(0, 0, 360, 470));
    ...
    public void SelectGroup() {
        /**
         *布置好朋友和Group菜单，这里还没有对朋友和group做分类
         * 看工作量要不要做
         */
        ...
        setFriendList();
        setGroupList();
        groupMerge.setBounds(new Rectangle(0, 170, 360, 500));
        this.add(groupMerge);
    }
    ...
    public void setSettings() {
        ...
        addFriend.setForeground(new Color(245, 245, 245));
        settings.add(addFriend);
        this.add(settings);
        settings.setBounds(0, 670, 360, 30);
        settings.setBackground(new Color(238, 238, 238));
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getClickCount() == 2) {
            ...
            try {
                /**
                 * 这里得到他的通信线程（通过管理通信线程类获得）
                 * 得到通信线程以后，通过得到socket去获取他的oos类，通过oos类发消息给服务器
                 */
                ObjectOutputStream oos=new ObjectOutputStream(ManageClientConServerThread.getClientConServerThread(owenerId).getS().getOutputStream());
                oos.writeObject(m);
            }catch (Exception ed){
    ...
    public void mouseEntered(MouseEvent e) {
        JLabel jLabel = (JLabel) e.getSource();
        jLabel.setForeground(new Color(245, 27, 38));
    }
